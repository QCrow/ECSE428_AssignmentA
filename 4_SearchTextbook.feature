...
Scenario: Search Results Include Matching Textbooks (Normal flow)
...
Scenario: Results Displayed in Order of Relevance (Normal flow)
...
Scenario: Search Results Include Thumbnail, Title, Author, ISBN, and Price (Normal flow)
  Given the user has performed a search for textbooks
  When the results are displayed
  Then each result should include a thumbnail image, title, author, ISBN, and price.

Scenario: No Results Found (Alternate flow)
  Given the user has performed a search for textbooks
