...
Scenario: Verify Adding Textbooks to Wishlist (Normal flow)
...
Scenario: Verify Viewing Wishlist (Alternate flow)
...
Scenario: Verify Removing Textbooks from Wishlist (Alternate flow)
Given the user has added textbooks to their wishlist
When the user removes a textbook from their wishlist
Then the textbook should be removed from the user's wishlist.

Scenario: Verify Saving Search Criteria (Alternate flow)
...
Scenario: Verify Error Message for Invalid Wishlist Addition (Error flow)
